...
try:
	...
	print "Waiting for TouchOSC Midi Port to be Created"
	while True:
		_in = mido.get_input_names()
		out = mido.get_output_names()
		try:
			inPort = [i for i, sf in enumerate(_in) if 'TouchOSC' in sf] 
			outPort = [i for i, sf in enumerate(out) if 'TouchOSC' in sf] 
			print "TouchOSC Midi Ports Found"
			midiIn = mido.open_input(_in[inPort[0]])
			midiOut = mido.open_output(out[outPort[0]])
			break
		except:
			print "No TouchOSC Port Found"
			sleep(10)
...
motorEN = True  #Global variable for enabling motors
run = False
pMidi = True	
MomentaryBP = False
minmax = ''
		...
		if ClearoneCommand['DeviceType'] == rxModel and ClearoneCommand['DeviceID'] == rxDeviceID and ClearoneCommand['Command'] == rxCommand and ClearoneCommand['Group'] == group and ClearoneCommand['Channel'] == channel:
			...
			if MomentaryBP:   #Check if a momentary button was presseed, cause then the value needs to be changed and sent back to the clearone
				...
				try:
					...
					print "Bad Button Data, probably moving to much at once"
				return
...
def setDefault():  #Get the stus of the clearone and sets the Midi Control Surface to match, runs at start up
	global motorEN
	global run
	motorEN = True
	run = False
	for C in Commands:
		...
		m = '#' + MidiCommand['DeviceType'] + MidiCommand['DeviceID'] + ' ' + MidiCommand['Command'] + ' ' + MidiCommand['Channel'] + ' ' + MidiCommand['Group']
		for i in MidiCommand['Values']:
			if MidiCommand['Values'][i][:2] == '*v' or MidiCommand['Values'][i][:2] == '*m' or MidiCommand['Values'][i][:2] == '*e':
				m = m + ' ' + default
			elif MidiCommand['Values'][i][:2] == '*f':
				m = m + ' ' + MidiCommand['Values'][i][2:]
			else:
				m = m + ' ' + MidiCommand['Values'][i]
